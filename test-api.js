...
const testAPI = async () => {
  try {
    ...
    try {
      ...
      logger.info(
        '✅ Resend API connection test: %s',
        connectionResponse.data.message
      );
    } catch (error) {
      logger.warn(
        '⚠️  Resend API connection test failed (expected if not configured): %s',
        error.message
      );
    }
    ...
    logger.info('\n📝 Next steps:');
    logger.info(
      '   1. Update .env file with your Resend API key and FROM_EMAIL'
    );
    logger.info(
      '   2. Test actual email sending with valid Resend configuration'
    );
